...
const Body=()=>{
    ...
    const [all_distributors_data,set_all_distributors_data]=useState(0);
    const [active_distributors_data,set_active_distributors_data]=useState(0);
    const [pending_approval_distributors_data,set_pending_approval_distributors_data]=useState(0);
    const [suspended_distributors_data,set_suspended_distributors_data]=useState(0);
    const [subscribed_distributors_data,set_subscribed_distributors_data]=useState(0);
	...
	const handle_get_distributors=async()=>{
		await Get_Distributors().then((response)=>{
            ...
            const stats_data = (data)=>{
                const get_total_distributors = data?.length;
                set_all_distributors_data(get_total_distributors);

                const get_total_active_distributors = data?.filter((item) => !item.suspension_status);
                set_active_distributors_data(get_total_active_distributors?.length);

                const get_total_suspended_distributors = data?.filter((item) => item.suspension_status);
                set_suspended_distributors_data(get_total_suspended_distributors?.length);

                const get_total_pending_approval_distributors = data?.filter(v => !v.verification_status)
                set_pending_approval_distributors_data(get_total_pending_approval_distributors?.length);

                const get_total_subscribed_distributors = data?.filter(v => v.subscription)
                set_subscribed_distributors_data(get_total_subscribed_distributors?.length);
			}
            stats_data(data);
                                ...
                                return(
                                    <Tr key={item?._id} bg={!item?.verification_status? 'gray.100':'none'} align={'center'} fontWeight={'regular'}>
                                        ...
                                        <Menu >
                                            ...
                                            <MenuList onClick={(()=>{router.push(`/suppliers_new/distributors/client/${item?._id}`)})}>
